...
export default async function AuditPage({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {rows.length === 0 ? (
        <p className="text-sm text-[var(--muted-foreground)]">Nothing matches those filters.</p>
      ) : (
            ...
            <tbody>
              {rows.map((row) => (
                <tr key={row.id} className="border-b last:border-b-0 align-top">
                  <td className="whitespace-nowrap px-4 py-2">{formatDateTime(row.createdAt)}</td>
                  <td className="px-4 py-2">{row.userEmail ?? "API key"}</td>
